...
const KEYS = {
  ...
  FINANCIAL_LEDGER: `karigar_financial_ledger_${STORAGE_VERSION}`
};
...
export const storageService = {
  ...
  /**
   * Financial Ledger & Khata Methods
   */
  loadFinancialLedger: (fallback = DEFAULT_FINANCIAL_TRANSACTIONS) => {
    try {
      const raw = safeGetItem(KEYS.FINANCIAL_LEDGER);
      if (!raw) {
        safeSetItem(KEYS.FINANCIAL_LEDGER, JSON.stringify(fallback));
        return fallback;
      }
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) {
  ...
  saveFinancialLedger: (transactions) => {
    try {
      if (!Array.isArray(transactions)) return false;
      safeSetItem(KEYS.FINANCIAL_LEDGER, JSON.stringify(transactions));
      return true;
    } catch (e) {
      console.error('[storageService] Failed to save financial ledger:', e);
      return false;
    }
  },
  ...
  addLedgerTransaction: (tx) => {
    try {
      const existing = storageService.loadFinancialLedger();
      const newTx = {
        id: tx.id || `tx-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
        date: tx.date || new Date().toISOString().slice(0, 10),
        description: tx.description || 'Craft Transaction',
      ...
      safeSetItem(KEYS.FINANCIAL_LEDGER, JSON.stringify(updated));
      return newTx;
    } catch (e) {
      console.error('[storageService] Failed to add ledger transaction:', e);
      return null;
    }
  },

  getFinancialSummary: (transactions = null) => {
    const list = transactions || storageService.loadFinancialLedger();
    let totalIncome = 0;
    let totalExpense = 0;
    ...
    // Credit Readiness Score based on digital transaction cadence & profit margin
    let baseScore = 650;
    if (list.length >= 5) baseScore += 50;
    if (list.length >= 10) baseScore += 30;
    if (netProfit > 0) baseScore += 50;
    if (profitMargin >= 30) baseScore += 40;
    if (list.some(t => t.category === 'B2B Wholesale')) baseScore += 30;
    const healthScore = Math.min(880, Math.max(500, baseScore));
...
export const DEFAULT_FINANCIAL_TRANSACTIONS = [
  {
    id: 'tx-1',
    date: '2026-09-08',
    description: 'B2B Wholesale Export Advance - FabCraft Global',
    type: 'income',
    category: 'B2B Wholesale',
    amount: 45000,
    paymentMethod: 'Bank Transfer (NEFT)',
    notes: '50% advance for 30 pure gold zari Chanderi sarees'
  },
  {
    id: 'tx-2',
    date: '2026-09-06',
    description: 'Raw Mulberry Silk Yarn (4kg) - Ashoknagar Co-op',
    type: 'expense',
    category: 'Raw Materials',
    amount: 8400,
    paymentMethod: 'UPI',
    notes: 'Grade-A 20/22 denier mulberry warp yarn'
  },
  {
    id: 'tx-3',
    date: '2026-09-04',
    description: 'Direct Marketplace Sale (3 Sarees to Bengaluru)',
    type: 'income',
    category: 'Direct Marketplace',
    amount: 16500,
    paymentMethod: 'UPI',
    notes: 'Order #KS-8210 direct-to-consumer delivery'
  },
  {
    id: 'tx-4',
    date: '2026-09-02',
    description: 'Testing Real Silver & Gold Zari Thread Spools',
    type: 'expense',
    category: 'Raw Materials',
    amount: 2600,
    paymentMethod: 'Cash',
    notes: 'GI certified zari threads for traditional booti motifs'
  },
  {
    id: 'tx-5',
    date: '2026-08-30',
    description: 'Pit-Loom Reed, Heald & Wooden Shuttle Overhaul',
    type: 'expense',
    category: 'Tool & Loom Maintenance',
    amount: 1500,
    paymentMethod: 'Cash',
    notes: 'Tuned by master cluster artisan blacksmith'
  },
  {
    id: 'tx-6',
    date: '2026-08-28',
    description: 'Apprentice Weaving Assistant Wages (Fortnight)',
    type: 'expense',
    category: 'Artisan Wages',
    amount: 4000,
    paymentMethod: 'Bank Transfer',
    notes: 'Fair wage compensation for 48 master-loom assist hours'
  },
  {
    id: 'tx-7',
    date: '2026-08-25',
    description: 'Handcrafted Packaging Boxes & Cloth Bags',
    type: 'expense',
    category: 'Packaging & Logistics',
    amount: 850,
    paymentMethod: 'UPI',
    notes: 'Eco-friendly kraft boxes with GI seal tags'
  },
  {
    id: 'tx-8',
    date: '2026-08-20',
    description: 'Exhibition Stall Sale - Bhopal Shilp Mela',
    type: 'income',
    category: 'Exhibition & Haat',
    amount: 12400,
    paymentMethod: 'UPI / Cash',
    notes: 'Sold 6 stoles & 4 running fabric cuts'
  }
];
