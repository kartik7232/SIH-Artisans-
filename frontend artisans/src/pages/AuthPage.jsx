...
export default function AuthPage() {
  ...
  const [selectedSignupRole, setSelectedSignupRole] = useState('artisan'); // 'artisan' | 'buyer' | 'admin'
  ...
  const demoProfiles = [
    ...
    },
    {
      ...
      cluster: 'KarigarAI Platform Core Team',
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Rule 1: Artisan / Seller detection (including Meena Devi / Chanderi)
      if (
        ...
        setConfidenceReason(
          lower.includes('meena') || lower.includes('chanderi')
            ? 'Recognized Master Artisan Meena Devi (Chanderi Handloom Cluster, MP - Seller / Master Artisan).'
            : 'Identified verified craft producer ID / rural mobile number pattern.'
        );
      } 
      // Rule 2: Developer / Admin detection (use whole word dev or developer, never substring match devi)
        ...
        setConfidenceReason('Recognized official KarigarAI developer domain or administrator identity.');
      } 
  ...
  const handleAuthSubmit = (e) => {
    ...
    if (mode === 'signup' && selectedSignupRole === 'admin' && developerKey.trim() !== 'KARIGAR_DEV_2026' && developerKey.trim() !== 'admin') {
      setAuthError('Invalid Developer Authorization Key. Contact the system administrator.');
      return;
    }

    // Determine final role with explicit artisan priority for Meena Devi
    let finalRole = mode === 'signin' ? (analyzedRole || 'buyer') : selectedSignupRole;
    if (
      lowerIdentifier.includes('meena') ||
      lowerIdentifier.includes('chanderi') ||
      lowerIdentifier.includes('artisan') ||
    ...
    <div className="unified-auth-page">
      ...
      <div className="container auth-container">
        ...
        <div className="auth-header-block">
          ...
          <h1 className="auth-title">Welcome to KarigarAI</h1>
              ...
              {mode === 'signup' && (
                <div className="form-group">
                  ...
                  <div className="role-selection-grid">
                    ...
                    {/* Role 3: Developer / Admin */}
                    <div 
                      className={`role-select-card ${selectedSignupRole === 'admin' ? 'selected' : ''}`}
                      onClick={() => setSelectedSignupRole('admin')}
                    >
                      <div className="role-card-header">
                        <Terminal size={20} color="#4B365F" />
                        <span className="role-name">Developer / Admin</span>
                      </div>
                      <p>Maintain web application, inspect AI pipeline telemetry, manage clusters, and verify artisans.</p>
                      {selectedSignupRole === 'admin' && <span className="active-dot">● Active</span>}
                    </div>
                  </div>
                </div>
              )}

              {/* Admin Security Key (Only in signup mode if admin selected) */}
              {mode === 'signup' && selectedSignupRole === 'admin' && (
                <div className="form-group dev-key-group">
                  <div className="label-with-hint">
                    <label className="field-label">Developer Authorization Passkey</label>
                    <span className="field-hint">Required for root maintainer access</span>
                  </div>
                  <div className="input-wrap">
                    ...
                    <input 
                      type="password"
                      className="text-input"
                      placeholder="Enter passkey (Hint: KARIGAR_DEV_2026 or admin)"
                      value={developerKey}
          ...
          <div className="auth-demo-sidebar">
            ...
            <div className="auth-guide-box">
              <div className="guide-title">
                <Compass size={16} />
                <span>How KarigarAI Role Routing Works:</span>
              </div>
